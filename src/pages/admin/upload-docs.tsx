...
function DocumentsUploadContent() {
  ...
  const stillProcessingCount = items.filter(it => it.status === 'reading' || it.status === 'extracting').length
  const needsTypeCount = items.filter(it => it.status === 'ready' && !it.detectedType).length
  // Blocks Send All until every file is past processing AND has a type
  // (auto-detected or hand-picked) — sending before that would silently
  // skip whatever hasn't finished identifying yet.
  const sendAllBlocked = stillProcessingCount > 0 || needsTypeCount > 0
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className={canSeeUploaded ? 'lg:col-span-2 card' : 'lg:col-span-3 card'}>
            ...
            {items.length > 0 && (
              <p className="text-xs text-gray-400 mt-3">{items.length} file{items.length !== 1 ? 's' : ''} · {readyCount} ready{needsTypeCount > 0 ? ` · ${needsTypeCount} need${needsTypeCount !== 1 ? '' : 's'} a type selected before Send All` : ''}</p>
            )}
          </div>
          )}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {items.length > 0 && (
                <div className="flex items-center gap-1.5">
                  <button onClick={startBatchSend} disabled={!!batchQueue || readyCount === 0 || sendAllBlocked}
                    title={sendAllBlocked ? "Every file needs a type (auto-detected or picked) before Send All unlocks" : "One Save/Mail/Notify/Reason choice, applied to every file — cusdec first, then cdn, then barcode, then boat note"} className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-medium text-white bg-[#1B3A5C] disabled:opacity-40">
